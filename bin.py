...
def bin_move_is_valid(bin_board, col):
    if bin_which_ROW(bin_board, col) <= 5:
        return True

def bin_game_done(bin_board):
    k = 0
    for i in range(COL_COUNT):
        if(bin_which_ROW(bin_board, i) <= 5):
            k+=1
            break
    if (k == 0):
        return True
...
def bin_valid_locations(bin_board):
    ...
    for c in range(COL_COUNT):
        if bin_move_is_valid(bin_board, c):
            valid.append(c)
    return valid

def bin_which_ROW(bin_board, col): ## next open row in given column
    return ((7<<(60-9*col))&bin_board)>>(60-9*col)

def bin_get_col_content(bin_board, col):
    content = [0, 0, 0, 0, 0, 0]
    for i in range(6):
        content[i] = ((1 << (59 - 9 * col - i) ) & (bin_board)) >> (59 - 9 * col - i)
    return content
...
def bin_to_np(bin_board):
    ...
    for i in range(7):
        rows_count[i] = bin_which_ROW(bin_board, i)
    cols_content = []
    for i in range(7):
        x = bin_get_col_content(bin_board, i)
        cols_content.append(x)
    for i in range(7):
        for j in range (rows_count[i]):
            new_np[j][i] = cols_content[i][j]+1
    return new_np
...
def bin_piece_drop(bin_board, row, col, piece):
    ...
    new_bin_which_ROW = bin_which_ROW(bin_board, col) + 1
